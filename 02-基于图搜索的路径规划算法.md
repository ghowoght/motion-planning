...
BFS是一种基于**队列(Queue)**的搜索算法，队列是一种**先进先出(FIFO)**的容器，如下图

<img src="img/02-图搜索/00-BFS-队列.png" alt="00-BFS-队列" style="zoom:80%;" />

以在下面的**无权图**中找到从结点`a`到结点`i`的路径为例，说明一下BFS算法的工作流程
...
从终点回溯，`i`的父结点为`f`，`f`的父结点为`e`，`e`的父结点为`a`，这样就可以得到`a`到`i`的最短路径为：`a`->`e`->`f`->`i`，如下

<img src="img/02-图搜索/00-BFS-流程-04.png" alt="00-BFS-流程-04" style="zoom:80%;" />

BFS中使用了大量的入队、出队操作，确保能找到最优路径，是一种以时间换空间的方法。

### DFS

与BFS不同，DFS是一种基于**栈(Stack)**的搜索算法，栈是一种**后进先出(LIFO)**的容器，如下图

<img src="img/02-图搜索/00-DFS-栈.png" alt="00-DFS-栈" style="zoom:80%;" />

其流程与上节中BFS类似，继续以上节的图举例，如下：
...
DFS中涉及的弹出操作比BFS少，是一种以时间换空间的方法，但是很显然找到的路径**并不是移动机器人运动规划所需要的最优路径**
...
BFS和DFS的区别主要在于结点的弹出策略，根据弹出策略的区别，分别使用了队列和栈两种数据结构，而栈和队列是两种相当基本的容器，



用一个函数估算当前结点到终点的距离，主要会用到欧氏距离(Euclidean Distance)和曼哈顿距离(Manhattan Distance)

但是在实际的地图中，常常会有很多障碍物

## Dijkstra算法

BFS只能用于无权图，下面我们要介绍一种针对带权图的图搜索算法
